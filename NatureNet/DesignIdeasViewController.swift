            ...
            if let idea = DataService.ds.GetDesignIdea(at: indexPath.row, searchFilter: searchText) {
                if let user = DataService.ds.GetUser(by: idea.submitter) {
                    ...
                    cell.configureCell(id: DESIGN_IDEAS_CELL_ID + "\(indexPath.section).\(indexPath.row)",
                        username: user.displayName, affiliation: DataService.ds.GetSiteName(with: user.affiliation), avatar: user.avatarUrl, text: idea.content, num_likes: "\(numLikes)", num_dislikes: "\(numDislikes)", num_comments: "\(numComment)", statusTxt: idea.status, date: idea.updatedAt, designIdea: idea, controller: self)
                }
            }
            return cell
        } else {
